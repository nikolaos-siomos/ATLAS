...
from helper_functions.printouts import print_header, print_subsection
    ...
    if len(path_keys) > 0:
        ...
        for key in unique_keys:
            ...
            if not isinstance(sig_raw, list):            
                ...
                metadata[meas_key] = {}
                metadata[meas_key]["system_info"] = system_info
                metadata[meas_key]["channel_info"] = channel_info
                metadata[meas_key]["time_info"] = time_info
                metadata[meas_key]["shots"] = shots
                metadata[meas_key]["mtype"]  = meas_type
        
        for key in path_keys:
            if key not in unique_keys:
                                
                meas_key = key.removeprefix("abs_")
                print_subsection(f"{meas_key} folder")

                data_key = [k for k, p in zip(unique_keys, unique_paths) if p == d[key]][0].removeprefix("abs_")
                
                loading_map[meas_key] = data_key

                # if data_key in profiles.keys():
                #     n_files = profiles[data_key].shape[0]
                #     print(f"-- Loading {n_files} file(s) from {data_key} dataset")
                    
                #     profiles[meas_key] = profiles[data_key]
                    
                #     metadata[meas_key] = {}
                #     metadata[meas_key]["system_info"] = metadata[data_key]["system_info"]
                #     metadata[meas_key]["channel_info"] = metadata[data_key]["channel_info"]
                #     metadata[meas_key]["time_info"] = metadata[data_key]["time_info"]
                #     metadata[meas_key]["shots"] = metadata[data_key]["shots"]
                #     metadata[meas_key]["mtype"]  = metadata[data_key]["mtype"]
