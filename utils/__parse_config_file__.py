...
SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "altitude":           {"dtype": float, "default": None, "is_list": False, "category": "optional", "min": 0, "max": 5000},
    "latitude":           {"dtype": float, "default": None, "is_list": False, "category": "optional", "min": -90,  "max": 90},
    "longitude":          {"dtype": float, "default": None, "is_list": False, "category": "optional", "min": -180, "max": 180},
    ...
    "background_low_bin":  {"dtype": int,   "default": None, "is_list": True,  "category": "recommended"},
    "background_high_bin": {"dtype": int,   "default": None, "is_list": True,  "category": "recommended"},
    ...
    "bins":                        {"dtype": int,   "default": None, "is_list": True, "category": "optional", "min": 1},
    "data_acquisition_range":      {"dtype": float, "default": None, "is_list": True, "category": "optional"},
    "analog_noise_scaling_factor": {"dtype": float, "default": None, "is_list": True, "category": "optional", "min": 0.,},
    "range_resolution":            {"dtype": float, "default": None, "is_list": True, "category": "optional", "min": 1., "max": 100.},
    "laser_repetition_rate":       {"dtype": float, "default": None, "is_list": True, "category": "optional", "min": 10., "max": 300.},
...
def _compute_emitted_wavelength_if_missing(parser_args: Dict[str, Any]) -> None:
    ...
    for i, ch in enumerate(parser_args["recorder_channel_id"]):
        ...
        if "." not in dt_wv or dt_wv.split(".")[-1] in ["", "0"]:
            print(f"--Warning: detected wavelength for channel {ch} is too round: {dt_wv}\nConsider providing it with accuracy of 2 decimal points (central wavelength of the interference filter)\n")
...
def _compute_background_bins_if_missing(parser_args: Dict[str, Any]) -> None:
    ...
    for i, ch in enumerate(parser_args["recorder_channel_id"]):
        ...
        if blo == None and bhi == None:
            if int(zbi) < -240:
                parser_args["background_low_bin"][i] = str(50)
                parser_args["background_high_bin"][i] = str(int(zbi) - 40)
            else:
                parser_args["background_low_bin"][i] = str(int(0.9*int(bns)))
                parser_args["background_high_bin"][i] = str(int(bns) - 50)
...
def read_ini_file(filepath: str)  -> Dict[str, Any]:
    ...
    for section in recognized_sections:
        if section not in config.sections():
            print(f"--Warning: {section} section not found in the configuration file")
...
def parse_atlas_config_file(filepath: str, debug: bool = False) -> Dict[str, Any]:
    ...
    # 3) Basic presence and uniquiness checks before expansion
    # recorder_channel_id must exist (mandatory) and not be empty
    _recorder_channel_id_check(parser_args)
    
    # 4) Expand channel lists using scalar defaults (including None) when omitted
    _expand_channel_lists_with_defaults(parser_args)

    # 5) Expand polarisation calibration lists using scalar defaults (including None) when omitted
    _expand_pol_cal_lists_with_defaults(parser_args)
    
    # 6) Length check polarisation calibratio
    ...
    # 7) Range & allowed checks (only on non-empty values)
    for name, meta in SCHEMA.items():
