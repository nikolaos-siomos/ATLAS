...
# -------------------------------------------------------------------
qa_tests = ['ray', 'pcb', 'tlc', 'tlc_rin', 'drk', 'trg', 'dtm', 'nsf', 'ray_pcb', 'pcb_aux', 'cam']
recognised_slicers = ['ray', 'pcb_p45', 'pcb_m45', 'tlc_north', 'tlc_east', 'tlc_south', 'tlc_west', 'tlc_rin_inner', 'tlc_rin_outer', 'trg', 'dtm', 'nsf', 'ray_pcb', 'pcb_aux_p45', 'pcb_aux_m45', 'drk', 'drk_ray', 'drk_pcb', 'drk_tlc', 'drk_tlc_rin', 'drk_trg', 'drk_dtm', 'drk_nsf', 'drk_ray_pcb', 'drk_pcb_aux']
...
SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "output_folder":       {"dtype": str,  "default": None,     "is_list": False, "category": "optional", "check_path": "dir"},
    ...
    "vertical_trimming":        {"dtype": float, "default": 32.,  "is_list": False, "category": "optional"},
    "trim_overflows":           {"dtype": int,   "default": 0,    "is_list": False, "category": "optional", "allowed": [0,1,2,3]},
    "ray_averaging_time":       {"dtype": str,   "default": None, "is_list": False, "category": "optional"},
    "ray_qck_averaging_time":   {"dtype": str,   "default": None, "is_list": False, "category": "optional"},
    ...
    "rsonde_column_units":   {"dtype": str,   "default": ['m_asl','hPa', 'K', 'percent'], "is_list": True, "category": "optional", "size": [3, 4], "allowed": height_units + pressure_units + temperature_units + humidity_units},
    "rsonde_latitude":       {"dtype": float, "default": None,     "is_list": False, "category": "optional", "min": -90,  "max": 90},
    "rsonde_longitude":      {"dtype": float, "default": None,     "is_list": False, "category": "optional", "min": -180, "max": 180},
    "rsonde_altitude":       {"dtype": float, "default": None,     "is_list": False, "category": "optional", "min": 0,  "max": 5000},
    "rsonde_station_name":   {"dtype": str,   "default": None,     "is_list": False, "category": "optional"},
    "rsonde_wmo_number":     {"dtype": int,   "default": None,     "is_list": False, "category": "optional", "min": 0},
}
...
def _special_checks(parser_args: Dict[str, Any]) -> None:
    ...
    example_text = "Only sets of 3 strings are accpetable where the first one is the test identifier (e.g. 'tlc'), the second is the start time in hhmm format (e.g. 2330) and the third is the stop time in hhmm format (e.g. 0100)"
            
    if slice_measurement != None:
    ...
    print("--Warning: Please note that since update 0.6.0 the more generic slice_measurement parameter can be provided instead of slice_rayleigh. The slice_rayleigh parameter will be ignored even if provided")
...
def _main_data_folder_check(parser_args: Dict[str, Any]) -> None:
...
def _special_path_handling(parser_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if parser_args.get("output_folder") == None:
        parser_args["output_folder"] = parser_args["parent_folder"]
    ...
    if all_none:
        sys.exit(f"Endpoint 1: No files to process! The QA test folders either don't exist or are empty. ATLAS terminates here")
...
def parse_call_atlas_ini(filepath: str, debug: bool = False) -> Dict[str, Any]:
    ...
    _main_data_folder_check(parser_args)
