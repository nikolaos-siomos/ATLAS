...
def _select_vldr_pairs(pol_cal_ratio, pol_cal_info, settings=None):
    """
    ...
    if pol_cal_info is not None and "ratio_type" in getattr(pol_cal_info, "parameters", []):
    ...
    # Optional user filtering through settings.  This keeps the routine useful
    # for large files without forcing all VLDR pairs to be plotted.
    requested_pairs = settings.get("pairs", settings.get("vldr_pairs", []))
    if requested_pairs not in [None, []]:
    ...
    for pair in selected:
        ...
        pair_records.append({"pair": pair, "ch_r": ch_r, "ch_t": ch_t})
...
def generate_vldr_quicklooks(data_pack, caller_info, settings_info):
    """
    Generate quicklook plots for time-resolved VLDR ratios.
    ...
    for key in process_qck:
        ...
        pair_records = _select_vldr_pairs(
        ...
        for record in pair_records:
            ...
            if ch_t is None:
                title = f"VLDR ratio {pair} ({ch_r})"
            else:
                title = f"VLDR ratio {pair} ({ch_r} / {ch_t})"
