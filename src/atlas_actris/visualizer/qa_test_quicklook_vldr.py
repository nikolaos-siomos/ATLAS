...
def generate_vldr_quicklooks(data_pack, caller_info, settings_info):
    ...
    data_pack[ray_key]['pol_cal_ratio'] instead of data_pack[ray_key]['profile'] and
    ...
    if ray_key not in data_pack.keys():
        return

    if 'pol_cal_ratio' not in data_pack[ray_key]:
        return

    if caller_info['vertical_scale'] not in data_pack[ray_key]:
        raise KeyError(
            f"Missing vertical scale '{caller_info['vertical_scale']}' "
            f"for VLDR quicklook key {ray_key}."
        )

    print_header(f"Start generating VLDR quicklooks ({ray_key})")

    # Use a distinct cleanup pattern so profile quicklooks are not removed.
    prepare_folder(caller_info, pattern="_qck_vldr_")

    ratios = data_pack[ray_key]['pol_cal_ratio']
    pol_cal_info = data_pack[ray_key].get('pol_cal_info')
    vertical_scale = data_pack[ray_key][caller_info['vertical_scale']]
    ...
    if len(pair_records) == 0:
        print(f"-- no VLDR pairs found for {ray_key}")
        print('-----------------------------------------')
        print(' ')
        return

    qa_test_info[ray_key] = {}
    ...
    for record in pair_records:
        ...
        metadata = _pair_or_channel_metadata(
            data_pack_key=data_pack[ray_key],
            pair=pair,
            ch_r=ch_r,
        )

        qa_test_info[ray_key][pair] = {
        ...
        lib = Libraries(
            ...
            extra_metadata={},
            settings=settings,
            qa_test_info=qa_test_info[ray_key][pair],
        )
        ...
        # except Exception:
        #     title = f"VLDR quicklook {ray_key} - {pair}"
        ...
        qa_test_info[ray_key][pair]['title'] = title
        qa_test_info[ray_key][pair]['filename'] = filename

        qa_test_info[ray_key][pair]['qck_plot_path'] = generate_plot(
            T=time,
            X=x_vals,
            Y=y_vals_sm,
            args=settings | qa_test_info[ray_key][pair] | caller_info,
        )

        perform_color_reduction(
            color_reduction=True,
            plot_path=qa_test_info[ray_key][pair]['qck_plot_path'],
        )
