...
default_values = {
    ...
    'absolute_extinction_error_threshold': 10.,
    'isolated_point_radius': 0.,
    'isolated_point_neighbour_threshold': 0
    }
...
def calculate_isolated_point_mask(mfit, win, mid, radius, neighbour_threshold):
    
    """
    Create a mask that removes isolated True points from an already-combined
    fit mask. A True point is considered isolated when it has fewer than
    neighbour_threshold True neighbours within radius bins in the 2D mask
    array. The point itself is not counted.
    
    The radius is interpreted in index/bin space, not in km. For example,
    radius = 2 checks array cells whose index distance from the tested cell
    is <= 2.
    """
    
    miso = np.ones_like(mfit, dtype = bool)
    
    if neighbour_threshold <= 0 or radius <= 0.:
        return(miso)
    
    mfit_bool = np.asarray(mfit, dtype = bool)
    true_idx = np.argwhere(mfit_bool)
    n_win, n_mid = mfit_bool.shape
    radius_int = int(np.ceil(radius))
    
    di_grid, dj_grid = np.meshgrid(
        np.arange(-radius_int, radius_int + 1),
        np.arange(-radius_int, radius_int + 1),
        indexing = 'ij'
        )
    
    neighbour_offsets = (di_grid ** 2 + dj_grid ** 2 <= radius ** 2) & \
                        ((di_grid != 0) | (dj_grid != 0))
    di_offsets = di_grid[neighbour_offsets]
    dj_offsets = dj_grid[neighbour_offsets]
    ...
    for idx in true_idx:
        ...
        ii = i + di_offsets
        jj = j + dj_offsets
        inside = (ii >= 0) & (ii < n_win) & (jj >= 0) & (jj < n_mid)
        neighbour_count = np.sum(mfit_bool[ii[inside], jj[inside]])
...
def create_masks(stats, keyw_args, cancel_stats, ccr_type,):
    ...
    ext_threshold = keyw_args['absolute_extinction_error_threshold']
    iso_radius = keyw_args['isolated_point_radius']
    iso_neighbour_threshold = keyw_args['isolated_point_neighbour_threshold']
    ...
    if 'ext' in cancel_stats: 
        ...
        mext = np.abs(vexe) <= ext_threshold
    ...
    mfit_initial = (msem) & (mder) & (msec) & (mshp) & (mccr) & (mpos) & (mdbw) & (mext)

    if 'iso' in cancel_stats:
        miso = np.ones_like(mfit_initial, dtype = bool)
    else:
        miso = calculate_isolated_point_mask(
            mfit = mfit_initial,
            win = win,
            mid = mid,
            radius = iso_radius,
            neighbour_threshold = iso_neighbour_threshold
            )

    mfit = mfit_initial & miso
    ...
    masks = {
        ...
        'durbin_watson' : mdbw,
        'is_positive' : mpos,
        'extinction_error' : mext,
        'isolated_points' : miso,
        'total' : mfit
        }
