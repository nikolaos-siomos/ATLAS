...
parent_folder = '/home/nikos/Nextcloud3/ARS-stations/the/179_199_664_20250505'
...
timescale = '2min' # Set to None to skip averaging. Use e.g. 10s for 10 second averages, 30min for 30 minute averages, or 1h for 1 hour averages. Use 'all' to average all files. Use None to apply no averaging


smoothing_window = 1000.#1000.#1000. #1. # in meters. Set to None to skip smoothing

smoothing_range = [2, 110.]#[0., 60.] #[2, 60] # in km. Set to None to smooth the whole profile. It will be applied only if the smoothing window is not None
...
background_range = [-5,-3]#[9., 60.] # in km. Set to None to skip background correction of each profile. It will be ignored if the signal type is set to 'rangecor'

statistics_range = [2,100.]# in km. Select a range to calculate statistics on signals to be displayed on the plots per channel. Statistics are calculated after averaging. Set to None to skip

dark_subtraction = True # Set to True to enable the dark signal subtraction. It can be applied only if the signal_type is 'raw', the mtype is not 'drk', and there is a drk file available in the netcdf folde
...
channels = ['1064xtax']# ['1064xpat'] # use the ATLAS IDs to plot only specific channels, can be a list or scalar, set to None to plot all channels
