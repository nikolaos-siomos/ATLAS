...
def _parse_group_section(
    ...
    *,
    ...
    for key, value in raw.items():
        ...
        if not entry_id:
            raise ValueError(f"[{section}] {key}: entry ID is missing")
        if parameter not in allowed_entry_parameters:
            raise ValueError(
                f"[{section}] {key}: unsupported entry parameter {parameter!r}; "
                f"expected one of {sorted(allowed_entry_parameters)}"
            )
        entries_raw.setdefault(entry_id, {})[parameter] = value
    ...
    for entry_id, mapping in entries_raw.items():
        # A completely empty named entry is a harmless template placeholder.
        if not any(str(value).strip() for value in mapping.values()):
            continue

        dataset_id = (mapping.get("dataset") or "").strip()
        if not dataset_id:
            raise ValueError(f"[{section}] {entry_id}.dataset: mandatory value is missing")
        if dataset_id not in dataset_ids:
            raise ValueError(
                f"[{section}] {entry_id}.dataset: unknown dataset ID {dataset_id!r}"
            )

        product_id = (mapping.get(id_key) or "").strip().lower()
        if not product_id:
            raise ValueError(f"[{section}] {entry_id}.{id_key}: mandatory value is missing")
        if product_id == "off":
            raise ValueError(
                f"[{section}] {entry_id}.{id_key}: 'off' is not used for named entries; "
                "remove the entry or leave all of its fields empty"
            )

        label = (mapping.get("label") or "").strip() or None
        reference_raw = (mapping.get("reference") or "").strip()
        reference = (
            _parse_bool(reference_raw, location=f"[{section}] {entry_id}.reference")
...
def _validate_group_reference_entry(
    *,
    section: str,
...
def parse_intercomparison_ini(filepath: str | Path) -> dict[str, Any]:
    ...
    for section, group_id in section_index["channel_group"]:
        ...
        values, entries = _parse_group_section(
            parser,
            section,
        ...
        reference_entry, reference_id = _validate_group_reference_entry(
            section=section,
            entries=entries,
