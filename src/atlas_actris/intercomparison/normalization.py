...
def _region_mean(
    ...
    *,
    ...
    # Keep the original region-selection behaviour unchanged: region values are
    # interpreted in km for the physical vertical scales and converted to metres
    # before comparison with the stored vertical coordinate.  This diagnostic
    # version only adds information; it does not change the normalization maths.
    ...
    if bool(np.asarray((n_bins == 0).all().values)):
        vertical_values = np.asarray(vertical.values, dtype=float)
        signal_values = np.asarray(value.values, dtype=float)
        finite_vertical = vertical_values[np.isfinite(vertical_values)]

        bins_in_region = np.asarray(mask.values, dtype=bool)
        n_coordinate_bins = int(np.count_nonzero(bins_in_region))

        print()
        print("    NORMALIZATION DIAGNOSTIC")
        print(f"        location: {location}")
        print(f"        vertical_scale: {vertical_scale}")
        print(f"        requested region: {region_km} km")
        print(f"        internal selection bounds: [{low_m}, {high_m}] m")
        print(f"        signal dims / shape: {value.dims} / {value.shape}")
        print(f"        vertical dims / shape: {vertical.dims} / {vertical.shape}")

        if finite_vertical.size:
            vmin = float(np.nanmin(finite_vertical))
            vmax = float(np.nanmax(finite_vertical))
            print(f"        vertical coordinate extent: [{vmin}, {vmax}] native units")
            if vertical_scale in {"range", "height_agl", "height_asl"}:
                print(f"        vertical coordinate extent: [{vmin / 1000.0:.6g}, {vmax / 1000.0:.6g}] km")
        else:
            print("        vertical coordinate extent: no finite values")

        print(f"        coordinate bins inside requested region: {n_coordinate_bins}")

        if n_coordinate_bins:
            try:
                region_values = np.asarray(value.where(mask).values, dtype=float)
                n_finite_region = int(np.count_nonzero(np.isfinite(region_values)))
            except Exception:
                n_finite_region = -1
            print(f"        finite signal values inside region: {n_finite_region}")

        n_finite_signal = int(np.count_nonzero(np.isfinite(signal_values)))
        print(f"        finite signal values in complete profile: {n_finite_signal}")

        # If possible, show the vertical span where the signal itself is finite.
        try:
            reduced_finite = np.isfinite(signal_values)
            while reduced_finite.ndim > 1:
                axis = 0 if reduced_finite.shape[0] != vertical_values.size else 1
                reduced_finite = np.any(reduced_finite, axis=axis)
            if reduced_finite.size == vertical_values.size and np.any(reduced_finite):
                vv = vertical_values[reduced_finite & np.isfinite(vertical_values)]
                if vv.size:
                    print(
                        "        vertical span with finite signal: "
                        f"[{float(np.nanmin(vv))}, {float(np.nanmax(vv))}] native units"
                    )
                    if vertical_scale in {"range", "height_agl", "height_asl"}:
                        print(
                            "        vertical span with finite signal: "
                            f"[{float(np.nanmin(vv))/1000.0:.6g}, "
                            f"{float(np.nanmax(vv))/1000.0:.6g}] km"
                        )
        except Exception:
            pass
...
def _reference_entry(group_kind: str, group_id: str, group: Mapping[str, Any]) -> dict[str, Any]:
    reference_id = group.get("reference_entry")
    reference = group.get("entries", {}).get(reference_id)
    if reference is None:
        raise ValueError(
            f"[{group_kind}:{group_id}] reference entry {reference_id!r} "
...
def _measured_reference_target(
    ...
    *,
    ...
    reference = _reference_entry(group_kind, group_id, group)
    ...
    if not isinstance(vertical, xr.DataArray):
        raise ValueError(
            f"[{group_kind}:{group_id}] reference entry has no "
...
def _channel_target(
    ...
    *,
    ...
    reference = _reference_entry("channel_group", group_id, group)
    ...
    if not isinstance(molecular, xr.DataArray):
        ...
        raise ValueError(
            ...
            f"reference entry has no molecular profile from source {source!r}."
        )
    if not isinstance(vertical, xr.DataArray):
        raise ValueError(
            f"[channel_group:{group_id}] reference entry has no "
            f"{vertical_scale!r} vertical metadata"
        )
...
def _apply_target_to_group(
    ...
    *,
    ...
    for dataset_id, dataset in group.get("entries", {}).items():
        ...
        if not isinstance(signal, xr.DataArray):
            raise ValueError(f"[{group_kind}:{group_id}] entry {dataset_id!r}: signal missing")
        if not isinstance(vertical, xr.DataArray):
            raise ValueError(
                f"[{group_kind}:{group_id}] entry {dataset_id!r}: "
        ...
        source_mean = _region_mean(
            ...
            vertical,
            region,
            location=f"[{group_kind}:{group_id}] entry {dataset_id!r} signal",
            vertical_scale=vertical_scale,
        )
...
def apply_intercomparison_normalization(
    ...
    if not channel_groups:
        ...
        for group_id, group in channel_groups.items():
            ...
            for dataset_id in group.get("entries", {}):
