...
def check_conflicts(options):
    ...
    if not options['normalization_range'] == None and options['signal_type'] == 'raw':
        CustomWarning("Normalization is not supported for raw signals. The provided normalization_range will be ignored")

    if not options['background_range'] == None and options['signal_type'] == 'rangecor':
        CustomWarning("Background correction is not supported for rangecorrected signals. The provided background_range will be ignored")
...
def get_fpaths(netcdf_folder, signal_type, mtype): 
    ...
    if signal_type == 'raw':   
        ...
        if len(files) == 0:
            raise Exception(f"--Warning: No raw files matching the given patern {prp_pattern} were detected!")
...
def multi_line_plot_bokeh(signal, signal_type, ranges = [], channels = None, 
                          ...
                          x_def_range = None, y_def_range_analog = None, y_def_range_photon = None):
    ...
    if x_def_range != None:
        ranges = ranges.sel({"bins" : slice(x_def_range[0],x_def_range[1])})
        signal = signal.sel({"bins" : slice(x_def_range[0],x_def_range[1])})
    ...
    for j in range(n_channel):
        ...
        if ch[6] == "a":
            if y_def_range_analog != None:
                y_range_1 = (y_def_range_analog[0], y_def_range_analog[1])
                print(y_range_1)
            else:
                y_range_1 = None
        
        if ch[6] == "p":
            if y_def_range_photon != None:
                y_range_1 = (y_def_range_photon[0], y_def_range_photon[1])
            else:
                y_range_1 = None
