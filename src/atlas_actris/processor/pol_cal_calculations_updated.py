...
def compute_calibration_factor(
    ...
    for target_key, (p45, m45) in assign_pcb.items():
        ...
        gain_products = {
            target_key: (eta_s, eta_s_error),
            p45: (eta_s_p45, eta_s_p45_error),
            m45: (eta_s_m45, eta_s_m45_error),
        }
        ...
        eta_s_f_p45_error = eta_s_p45_error / np.abs(trans_ratio)

        eta_s_f_m45 = eta_s_m45 / trans_ratio
        eta_s_f_m45_error = eta_s_m45_error / np.abs(trans_ratio)
        ...
        # Epsilon is only stored as a regional statistic in pol_cal_info for
        # the combined pcb/pcb_aux eta entries. No epsilon profile is stored.
        epsilon = epsilon_angle(
            eta_p45=eta_s_f_p45,
            eta_m45=eta_s_f_m45,
            kappa=1.0,
        )
        epsilon = epsilon.assign_coords(pair=eta_ids).rename("epsilon")
        epsilon_m_bins = mean_in_region(
            da=epsilon,
            z=z_pair_p45.assign_coords(pair=eta_ids),
            averaging_range=averaging_range,
        )
        ...
        eta_p45 = eta_s_f_p45 / K
        eta_p45_error = eta_s_f_p45_error / np.abs(K)

        eta_m45 = eta_s_m45 / K
        eta_m45_error = eta_s_f_m45_error / np.abs(K)
        ...
        for spec in product_specs:
            ...
            for key, (values, errors) in spec["products"].items():
                # Keep the time-free arrays for region statistics, because
                # pol_cal_info cannot store values with a time dimension.
                ...
                if key == target_key and ratio_type == "eta":
                    info_with_stats = add_parameter(
                        pci=info_with_stats,
                        name="epsilon",
                        values=epsilon_m_bins,
                    )

                # Store calibration products as time-free arrays.  These are
                # calibration constants/profiles and should not carry a
...
def compute_mldr(
    ...
    Channel combinations are taken from existing eta entries under the combined
    pcb/pcb_aux calibration keys, matching compute_calibrated_ratio.  The
    molecular input is read from output_data["molec"] instead of
    output_data["profile"] and is expected to have dims (channel, bins).

    The output is saved under output_data["pol_cal_ratio"]["ray_pcb"] with:
        ratio_type = "mldr"
        pair id type index = "m"

    No random error is calculated or stored.  The regional mean is stored in
    pol_cal_info; no sem row is added.
    """
    ...
    ch_r, ch_t, eta_ids, eta_info = _collect_eta_pairs_from_pcb(output_data)
    if len(eta_ids) == 0:
        print_entry("MLDR calculation skipped: no pcb/pcb_aux eta entries found.")
        return output_data
    ...
    if len(keep) == 0:
        print_entry("MLDR calculation skipped: no eta channel pairs found in ray_pcb molecular profiles.")
        return output_data
    ...
    mldr_ids = [
        replace_ratio_id_type(eta_id, old_type_index="e", new_type_index="m")
        for eta_id in eta_ids
    ]
    ...
    molec_ratio = simple_ratio(
        numerator=molec_r,
        denominator=molec_t,
        info=mldr_info,
    )

    eta, _ = _find_eta_for_pairs(
        output_data=output_data,
        ch_r=ch_r,
        ch_t=ch_t,
        calibrated_pair_ids=mldr_ids,
    )

    pair_ids_found = list(eta.pair.values)
    molec_ratio = molec_ratio.sel(pair=pair_ids_found)
    mldr_info = mldr_info.sel(pair=pair_ids_found)

    ch_r_found = mldr_info.sel(parameters="ch_r").values.tolist()
    ch_t_found = mldr_info.sel(parameters="ch_t").values.tolist()

    calibrated_molec_ratio = molec_ratio / eta
