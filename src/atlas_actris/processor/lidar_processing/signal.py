    ...
    for ch in channels:
        ...
        if ch[6] == 'p': #7th digit of channel name is the acquisition mode (a or p)
            ...
            if mask_saturated.any():
                CustomWarning(f"Channel {ch} - Photon signal countrate values above the 95% of the maximum allowed value were detected! ")
        ...
        if ch[6] == 'a': #7th digit of channel name is the acquisition mode (a or p)
            ...
            if mask_saturated.any():
                CustomWarning(f"Channel {ch} - Analog signal mV values above the 95% of the data acqusition range were detected! ")   
...
def dead_time_correction(sig, dead_time, dead_time_cor_type):
    ...
    if (dead_time_cor_type.values == 1).any():
        CustomWarning("A Dead time correction for Paralyzable system not implemented yet!")
    ...
    for ch in channels:
        ...
        if ch[6] == 'p': #7th digit of channel name is the acquisition mode (a or p)
            ...
            if (sig.loc[ch_d].values > 1000./dead_time[ch]).any():
                CustomWarning(f"Channel {ch} - A countrate value above the maximum allowed value was detected! Consider revising the input file ")
