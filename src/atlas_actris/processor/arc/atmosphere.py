...
def find_lower_indices(H, heights):
    H = np.asarray(H)
    heights = np.asarray(heights)

    # searchsorted(H, x) returns the insertion index where x would fit
    # subtract 1 → gives the index of the closest LOWER or EQUAL value
    idx = np.searchsorted(H, heights, side="right") - 1

    # clamp negative values to 0
    idx[idx < 0] = 0

    # clamp too-large values (for heights >= H[-1])
    idx[idx >= len(H)] = len(H) - 1

    return idx

    
def linear_pressure_interpolation(H, P, heights):
 
    P_f = interp1d(H, P, bounds_error = False, fill_value = np.nan)
    
    P_interp = P_f(heights)
    
    return(P_interp)

def logarithmic_pressure_interpolation(H, P, heights):
 
    P_f = interp1d(H, np.log(P), bounds_error = False, fill_value = np.nan)
    
    P_interp = np.exp(P_f(heights))
