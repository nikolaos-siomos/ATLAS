...
def compute_gain_ratio(
    ...
    """
    Compute gain ratios for pcb/pcb_aux +/-45 QA tests and create the combined
    pcb/pcb_aux gain-ratio products.

    The reflected and transmitted signals of the individual +/-45 tests are
    averaged over time before the ratio is formed.  The saved products are
    time-free arrays with dimensions (pair, bins).

    In addition to the source keys
        pcb_p45, pcb_m45, pcb_aux_p45, pcb_aux_m45
    this function also creates
        pcb, pcb_aux
    by combining the matching +45 and -45 gain-ratio products as
        eta_s = sqrt(gain_p45 * gain_m45).

    All products created here use:
    ...
    def _add_gain_stats(
        ...
        info = add_parameter(info, name="ratio_type", values="gain_ratio")
        return info

    def _store_gain_product(
        key: str,
        values: xr.DataArray,
        errors: xr.DataArray,
        info: xr.DataArray,
    ...
    for qa_test in allowed_qa_tests:
        ...
        pci = _add_gain_stats(
            info=pci,
            values=ratio_da.rename("ratio"),
            errors=ratio_error_da.rename("ratio_error"),
            z_pair=z_pair,
        )
    ...
    # ------------------------------------------------------------------
    # 2) Combined pcb/pcb_aux gain ratios.  These are the eta_s products that
    #    compute_calibration_factor() will use as its starting point.
    # ------------------------------------------------------------------
    assign_pcb = {
        "pcb": ("pcb_p45", "pcb_m45"),
    ...
    for target_key, (p45, m45) in assign_pcb.items():
        ...
        # Make the combined calibration key behave like a real qa_test for
        # downstream calibration calculations.
        if target_key not in vertical_scale:
            vertical_scale[target_key] = vertical_scale[p45]
...
def compute_calibration_factor(
    ...
    """
    Compute eta_s_f and eta from the combined pcb/pcb_aux gain ratios.

    The combined pcb/pcb_aux gain-ratio products are now created upstream by
    compute_gain_ratio() and represent eta_s.  This function therefore no
    longer combines pcb_p45/pcb_m45 itself.  It starts from:

        output_data["pol_cal_ratio"]["pcb"]
        output_data["pol_cal_ratio"]["pcb_aux"]

    using their ratio_type="gain_ratio" entries as eta_s, and stores:
        eta_s_f : eta_s corrected by R_to_T, using f IDs
    ...
    for target_key in ["pcb", "pcb_aux"]:
        ...
        if target_key not in vertical_scale:
            CustomWarning(
                ...
                f"no combined vertical scale found."
            )
            continue
        ...
        eta_s = pol_cal_ratio[target_key].sel(pair=gain_ids)
        eta_s_error = pol_cal_ratio_error[target_key].sel(pair=gain_ids)

        eta_s, eta_s_error = _time_mean_and_error(
            eta_s,
            eta_s_error,
        )
        ...
        product_specs = [
            {
                ...
                "products": {target_key: (eta_s_f, eta_s_f_error)},
                "z_pair": z_pair_gain.assign_coords(pair=eta_s_f_ids),
            },
            {
                "ratio_type": "eta",
                "pair_ids": eta_ids,
                "products": {target_key: (eta, eta_error)},
                "z_pair": z_pair_gain.assign_coords(pair=eta_ids),
            },
        ]
        ...
        for spec in product_specs:
            ...
            for key, (values, errors) in spec["products"].items():
                values_for_stats = values.assign_coords(pair=pair_ids).rename("ratio")
                errors_for_stats = errors.assign_coords(pair=pair_ids).rename("ratio_error")
...
def compute_calibrated_ratio(
    ...
    function leaves the input unchanged.  The ray_pcb output remains
    time-resolved; any time averaging is expected to happen in an external
    averaging stage.
    """
    ...
    calibrated_error_m_bins = sem_in_region(
        da_error=calibrated_error,
        z=z_pair,
        averaging_range=processing_info["settings_info"]["pcb"]["rayleigh_region"],
    )

    # mean/sem retain the time dimension for time-resolved ray_pcb data; do not
    # store them in pol_cal_info.  Time averaging, if desired, is external.
    if "time" not in calibrated_m_bins.dims:
...
def compute_vldr(
    ...
    VLDR is generated only from existing ray_pcb entries with
    ratio_type="calibrated_ratio".  The output remains time-resolved; any time
    averaging is expected to happen externally.
    """
    ...
    # mean/sem retain time for time-resolved ray_pcb data; do not store them in
    # pol_cal_info unless the input has already been externally time-averaged.
    if "time" not in vldr_m_bins.dims:
