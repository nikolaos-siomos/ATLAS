...
def main(mst_args):
    ...
    if not np.isin(np.array(process_qck),np.hstack((process,"off"))).all():
        if process != 'off':
            CustomWarning("The provided process_qck option {process_qck} is not a subset of the provided process option {process}. Only the corresponding quicklooks will be plotted")
