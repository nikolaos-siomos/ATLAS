...
def _raise_unknown_config_parameter_errors(config: configparser.ConfigParser) -> None:
    """Raise ConfigError if the INI file contains keys outside SCHEMA.
    ...
    for section in config.sections():
        for key in config[section].keys():
            ...
            if key_str in schema_keys:
                continue

            entry = (section, key_str)
            unknown.append(entry)

            if key_str in RENAMED_CONFIG_KEYS:
                renamed.append((section, key_str, RENAMED_CONFIG_KEYS[key_str]))
    ...
    msg = (
        "Configuration file contains parameter(s) that are not defined in "
        "the ATLAS configuration schema. Empty declarations are also invalid.\n"
    ...
    if renamed:
        ...
        msg += (
            "\n\nSince version 1.0.0, some configuration parameters were renamed:\n"
            "  - daq_trigger_offset has been renamed to zero_bin\n"
            "  - laser has been renamed to laser_id\n"
            "  - altitude has been renamed to station_altitude\n"
            "  - longitude has been renamed to station_longitude\n"
            "  - latitude has been renamed to station_latitude\n"
            "\nPlease update the following renamed parameter(s):\n"
            f"{renamed_lines}"
        )
...
def read_ini_file(filepath: str)  -> Dict[str, Any]:
    ...
    # Preserve option/key case. ConfigParser lowercases option names by default,
    # which would turn schema keys like G and H into g and h.
    ...
    _raise_unknown_config_parameter_errors(config)
...
def _normalize_entries(parser_args: Dict[str, Any]) -> None:
    """Normalize only keys that belong to the ATLAS config schema.

    Strict unknown-key validation is intentionally done only in read_ini_file(),
    where the raw user configuration file is parsed.  Later processing steps may
    merge extra metadata/header keys into config_info, for example
    analog_to_digital_resolution.  Those keys should not make runtime
    normalization fail just because they are not part of SCHEMA.
    """
