...
def _main_data_folder_check(parser_args: Dict[str, Any], filepath: str) -> Dict[str, Any]:

    main_data_folder = parser_args.get("main_data_folder")

    if main_data_folder is None:
        print(
            "-- The main_data_folder was not provided. By default it will be "
            "assigned to the parent folder of the call_atlas.ini file.\n"
        )
        parser_args["main_data_folder"] = os.path.dirname(filepath)

    else:
        for key in explicit_path_keys:
            if parser_args.get(key) is not None:
                CustomWarning(
                    f"Both {key} and the main_data_folder were provided. "
                    "Automatically detected paths are always overridden by explicit paths"
                )
...
def _special_path_handling(parser_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if path_ray_pcb is None and path_ray is not None and path_pcb is not None:
...
def parse_call_atlas_ini(filepath: str, debug: bool = False) -> Dict[str, Any]:
    ...
    # 3) Check if the main_data_folder exists, use any explicitly defined path
    # instead of trying to automatically detect it later
