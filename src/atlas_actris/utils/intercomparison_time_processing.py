...
def _mean_signal(
    ...
    *,
    ...
    return filtered.mean(dim="time", skipna=True), {
        "mode": "filtered_mean",
        "n_before": n_before,
        "n_after": n_after,
    }
...
def _process_group_collection(
    ...
    *,
    ...
    for group_id, group in groups.items():
        ...
        for dataset_id, dataset in group_out.get("datasets", {}).items():
            base = f"[{group_kind}:{group_id}] dataset {dataset_id!r}"
            ...
            if isinstance(molecular, dict):
                molecular["profile"], _ = _mean_signal(
                    molecular.get("profile"),
                    slice_intervals=slice_intervals,
                    exclude_intervals=exclude_intervals,
                    location=f"{base} molecular profile",
                )
...
def filter_and_average_intercomparison_bundles(
    ...
    tuples. The same global windows are applied to every participating dataset.
