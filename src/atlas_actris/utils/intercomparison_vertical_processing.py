...
def _common_bounds(
    ...
    *,
    ...
    if not bounds:
        raise ValueError(f"[{group_kind}:{group_id}] contains no participating datasets")
    ...
    if not low < high:
        raise ValueError(
            f"[{group_kind}:{group_id}] datasets have no common valid overlap "
...
def _automatic_group_bin_width(
    ...
    *,
    ...
    for dataset_id, dataset in group.get("datasets", {}).items():
        vertical = _as_1d_vertical(
            dataset.get("metadata", {}).get(vertical_scale),
            location=f"[{group_kind}:{group_id}] dataset {dataset_id!r} {vertical_scale!r}",
        )
        steps[dataset_id] = _nominal_native_step(
            vertical,
            location=f"[{group_kind}:{group_id}] dataset {dataset_id!r} {vertical_scale!r}",
        )
...
def _combine_group_arrays(group_kind: str, group_id: str, group: dict[str, Any]) -> None:
    """Create aligned group arrays with dimensions ``(dataset, bins)``."""
    ...
    def _combine(key: str) -> Optional[xr.DataArray]:
        ...
        combined = xr.concat(arrays, dim=xr.IndexVariable("dataset", labels), join="exact")
    ...
    for dataset_id, dataset in datasets.items():
        ...
        molecular_labels.append(dataset_id)

    if molecular_arrays:
        molecular = xr.concat(
            molecular_arrays,
            dim=xr.IndexVariable("dataset", molecular_labels),
            join="exact",
        )
        z = np.asarray(vertical_grid.values)
        if vertical_scale == "bins":
...
def _harmonize_group_interpolation(
    ...
    *,
    ...
    The target grid is taken from the participating dataset with the largest
    ...
    # Keep dataset order deterministic when two datasets have the same
    # coarsest nominal step.
    tolerance = max(1.0e-12, abs(worst_step) * 1.0e-9)
    target_dataset_id = next(
        dataset_id
        for dataset_id in datasets
        if abs(native_steps[dataset_id] - worst_step) <= tolerance
    )
    target_dataset = datasets[target_dataset_id]
    ...
    target_native_vertical = _as_1d_vertical(
        ...
        location=(
            f"[{group_kind}:{group_id}] interpolation target dataset "
            f"{target_dataset_id!r} {vertical_scale!r}"
        ),
    )
    ...
    if target_vertical.size < 2:
        raise ValueError(
            f"[{group_kind}:{group_id}] coarsest native grid from dataset "
            f"{target_dataset_id!r} contains fewer than two points inside the "
    ...
    for dataset_id, dataset in datasets.items():
        ...
        _interpolate_dataset(
            ...
            vertical_scale=vertical_scale,
            location=f"[{group_kind}:{group_id}] dataset {dataset_id!r}",
        )
    ...
    group["interpolation_target_dataset"] = target_dataset_id
    group["interpolation_target_step"] = _from_internal_units(
        worst_step, vertical_scale
    )
    group["native_vertical_steps"] = {
        dataset_id: _from_internal_units(step, vertical_scale)
        for dataset_id, step in native_steps.items()
    }
...
def _harmonize_group_binning(
    ...
    *,
    ...
    if requested_width is None:
        ...
        # one participating dataset does not actually provide.
    ...
    for dataset_id, dataset in group.get("datasets", {}).items():
        ...
        if isinstance(signal, xr.DataArray):
            before[dataset_id] = signal.sizes.get("bins")
    ...
    if native_steps:
        for dataset_id, step in group["native_vertical_steps"].items():
            print(
                f"        {dataset_id}: native step={step:.6g} "
                f"{_units_label(vertical_scale)}; {before.get(dataset_id, '?')} -> {centers.size} bins"
            )
    else:
        for dataset_id in group.get("datasets", {}):
            print(f"        {dataset_id}: {before.get(dataset_id, '?')} -> {centers.size} bins")
        ...
        for group_id, group in groups.items():
            if vertical_method == "interpolation":
                ...
                print(
                    ...
                    f"from {group['interpolation_target_dataset']!r} "
                    f"(step={group['interpolation_target_step']:.6g} "
                    f"{_units_label(vertical_scale)}, "
                    f"{group['vertical_grid'].sizes['bins']} bins)"
                )
                for dataset_id, step in group.get("native_vertical_steps", {}).items():
                    print(
                        f"        {dataset_id}: native step={step:.6g} "
                        f"{_units_label(vertical_scale)}"
                    )
