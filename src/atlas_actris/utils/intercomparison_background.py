...
def _reference_background_region_from_metadata(
    ...
    reference_dataset = group.get("reference_dataset")
    reference = group.get("datasets", {}).get(reference_dataset)
...
def _background_correct_dataset(
    ...
    *,
    ...
    ranges = dataset.get("metadata", {}).get("range")
    ...
    if not isinstance(ranges, xr.DataArray):
        raise ValueError(f"{location}: range metadata are required for background correction")

    low_m = 1000.0 * float(region_km[0])
    high_m = 1000.0 * float(region_km[1])
    mask = (ranges >= low_m) & (ranges <= high_m)
        ...
        for group_id, group in channel_groups.items():
            ...
            for dataset_id, dataset in group.get("datasets", {}).items():
                _background_correct_dataset(
                    dataset,
                    region_km=region,
                    location=f"[channel_group:{group_id}] dataset {dataset_id!r}",
                )
        ...
        for group_id, group in pair_groups.items():
            ...
            for dataset_id, dataset in group.get("datasets", {}).items():
                _background_correct_dataset(
                    dataset,
                    region_km=region,
                    location=f"[pair_group:{group_id}] dataset {dataset_id!r}",
                )
                print(f"        {dataset_id}: background estimated and subtracted")
    else:
