...
def check_range(da, name, minv, maxv):
    bad = (da < minv) | (da > maxv)
    if bad.any():
        raise Exception(f"{name} out of range [{minv}, {maxv}]\n"+
                        "Values:", da.where(bad, drop=True).values +
                        "\nPlease make sure th radiosonde parsing options are correct")
        
def height_binning(z, F, resolution, dim = 'height'):
    
    bins = np.arange(z.min(), z.max() + resolution, resolution)

    z = z.copy().groupby_bins("height", bins).mean().rename({"height_bins": "height"})
    F = F.copy().groupby_bins("height", bins).mean().rename({"height_bins": "height"})
    
    bin_centers = [b.mid for b in z[dim].values]
    
    z = z.assign_coords(height=bin_centers)
    F = F.assign_coords(height=bin_centers)
    
    return(z, F)

def sanity_check_atmo(atmo):
    P = 1e3 * atmo.sel(parameters="P")  # Pa
    T = atmo.sel(parameters="T")  # K
    z = atmo["height"]            # m

    height_bins_T = np.arange(z.min(), z.max() + 1000., 1000.)

    z_T = z.copy().groupby_bins("height", height_bins_T).mean().rename({"height_bins": "height"})
    T_sm = T.copy().groupby_bins("height", height_bins_T).mean().rename({"height_bins": "height"})

    bin_centers_T = [b.mid for b in z_T.height.values]


    z_T = z_T.assign_coords(height=bin_centers_T)
    T_sm = T_sm.assign_coords(height=bin_centers_T)

    # 2) Monotonic pressure
    if any(np.gradient(P, z, edge_order=2) > 0):
        CustomWarning("Pressure derivative is not decreasing with height. This could be due to noise")
        print("--Smoothing by 100 m and trying again")

        z_P, P_sm = height_binning(z, P, resolution = 100., dim = 'height')

        if any(np.gradient(P_sm, z_P, edge_order=2) > 0): 
            raise Exception("Pressure derivative is not decreasing with height even after smoothing. Please revise the radiosonde/model file")
        else:
            print("    Pressure derivative test succeded after smoothing!")
            

    if sum(np.gradient(z, edge_order=2) < -5) > 10:

        raise Exception("Radiosonde height was found to be decreasing by more than 5 meters more than 10 times")
        
    # 3) Lapse rate
    Gamma = -np.gradient(T, z, edge_order=2) * 1000 

    if (Gamma > 25).any():
        CustomWarning("Temperature lapse rate > 25 K/km. This could be due to noise.")
        print("--Smoothing by 1000 m and trying again")
        
        z_T, T_sm = height_binning(z, T, resolution = 1000., dim = 'height')
        Gamma_sm = -np.gradient(T_sm, z_T, edge_order=2) * 1000 

        if any(Gamma_sm > 25): 
            raise Exception("Temperature lapse rate > 25 K/km even after smoothing. Please revise the radiosonde/model file")

        else:
            print("    Temperature lapse rate test succeded after smoothing!")
            
    if (Gamma < -12).any():
        
        CustomWarning("Temperature lapse rate < -12 K/km. This could be due to noise.")
        print("--Smoothing by 1000 m and trying again")
        ...
        if any(Gamma_sm < -12): 
            raise Exception("Temperature lapse rate < 12 K/km even after smoothing. Please revise the radiosonde/model file")
        else:
            print("    Temperature lapse rate test succeded after smoothing!")
...
def radiosonde(finput_rs, delimiter, skip_header, skip_footer, 
    ...
    check_range(atmo.height, "Radiosonde/Model altitude", 0, 100000)
    check_range(atmo.sel(parameters="P"), "Radiosonde/Model pressure", 0, 1200)
    check_range(atmo.sel(parameters="T"), "Radiosonde/Model temperature", 150, 340)
    
    sanity_check_atmo(atmo)
