...
# Channels
        if parser.has_section('Channels'):
            ...
            if channel_section.recorder_channel_id.is_unique:
                channels = channel_section.recorder_channel_id.values

            else:
                if 'laser' not in channel_section.columns.values:
                    raise Exception("-- Error: recorder_channel_id contains duplicate indices and the laser id is not provided in the configuration file. Please provided the laser id according to the raw file header info.\nrecorder_channel_id: {channel_section.recorder_channel_id.values}")
                else:                    
