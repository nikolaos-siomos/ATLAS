...
def read_meas(raw_data):
    ...
    station_latitude = raw_data.location_coordinates.values[0]
    station_longitude = raw_data.location_coordinates.values[1]
    zenith_angle = raw_data.zenithangle.values
    
    system_info = pd.Series()
    
    system_info['station_altitude'] = raw_data.location_height.values   
    
    system_info['station_latitude'] = np.round(station_latitude, decimals = 4)
    system_info['station_longitude'] = np.round(station_longitude, decimals = 4)
    
    system_info['zenith_angle'] = np.round(zenith_angle, decimals = 2)
...
def read_channels(raw_data):
    ...
    channel_info.loc[:,'laser_id'] = "1" # only 1 laser
...
def get_cal_info(pol_cal_angle, meas_type):
    ...
    if meas_type not in ['nrm','pcb_p45','pcb_m45']:
        ...
        fix_pos = find_fixed_pos(pol_cal_angle, meas_type)
...
def check_cal_angle(pol_cal_angle):
    ...
    if not mask_err.all():
        raise FileReaderError(f'-- Error: The absolute pol. calibrator angle provided in the raw files above 360 degrees without being 999. For example {pol_cal_angle[~mask_err]}. The value 999 is allowed in some old polly_xt systems showing that the calibrator is at the zero position')
    
    return()

def find_fixed_pos(pol_cal_angle, meas_type):
    ...
    if sorted_uniques.size == 1 and meas_type in ["pcb_p45", "pcb_m45"]:
    ...
    elif sorted_uniques.size == 2 and meas_type not in ["pcb_p45", "pcb_m45"]:
        raise FileReaderError('-- Error: The provided measurement does not seem to include only calibration profiles. Please correct the input file')
...
def assign_cal_positions(fix_pos):
    ...
    if len(fix_pos) == 3:
        ...
        if np.sum(mask_p45_pos) == 0:
            raise FileReaderError(f'-- Error: It is not possible to assume the +45 calibrator position because the absolute calibrator angle in any of the 2 fixed non zero positions {fix_pos} is not within 0 and 90 degrees nor within 180 and 270 degrees. Please correct the input file or report this issue to CARS if not necessary ')
        elif np.sum(mask_p45_pos) > 1:
            raise FileReaderError(f'-- Error: It is not possible to assume the +45 calibrator position because the absolute calibrator angle is within 0 and 90 degrees or within 180 and 270 degrees in both fixed non zero positions {fix_pos}. Please correct the input file or report this issue to CARS if not necessary ')
        else:
...
def read_dataset(dir_meas, meas_type, lazy=True, chunks=None):
    """Read raw PollyXT netCDF files.

    Parameters
    ----------
    dir_meas : str
        Folder containing PollyXT netCDF files.
    meas_type : str
        Measurement type used by the existing PollyXT calibration-mask logic.
    lazy : bool, optional
        If True, ``sig_raw`` remains Dask-backed and the raw signal is not
        loaded into memory during reading. If False, the old eager behaviour is
        used.
    chunks : dict or None, optional
        Reader-level xarray chunks. By default this follows the lazy Licel
        reader: time chunks of 5, channel chunks of 5, and full-bin chunks.
    """
    ...
    if not(os.path.exists(dir_meas)):
        ...
        if len(mfiles) > 0:
            ...
            if "raw_signal" not in raw_data:
                raise FileReaderError("raw_signal parameter not found in the netcdf file. This is not a polly_xt raw file")
            ...
            for k in range(len(mfiles)):
                ...
                if lazy:
                    raw_data = xr.open_dataset(mfiles[k], chunks={})
                else:
                ...
                if meas_type == 'pcb_p45':
                    mask = mask_p45    
                elif meas_type == 'pcb_m45':
                    mask = mask_m45 
                else:
                    mask = mask_zer                    
                ...
                if mask.any():
                    if lazy:
                        raw_signal = raw_data.raw_signal[mask,:,:].astype(float).data
                    else:
            ...
            if len(list_sig) > 0:
                ...
                # Sort by time
                # Avoid copy() when lazy so the Dask-backed signal remains lazy.
                if lazy:
