...
def read_single_matlab_signal(fname):
    """Read one MATLAB Licel signal block lazily through dask.delayed."""

    data = loadmat(fname)
    dataKeys = [key for key in data.keys() if key.startswith('set')]
    body = np.array([data[dataKeys[i]] for i in range(len(dataKeys))])
    return body[:, 0, :].astype(float)
    ...
    if not(os.path.exists(dir_meas)):
        ...
        if len(mfiles) > 0:
            ...
            for k in range(len(mfiles)):
                ...
                sig_delayed = delayed(read_single_matlab_signal)(mfiles[k])
            ...
            sig_raw = unit_conv_MHz_to_counts(signal = sig_raw, shots = shots, channel_info = channel_info)
...
def read_channels(buffer):
    ...
    info_columns = ['acquisition_mode', 'laser_id', 'bins', 
...
def unit_conv_MHz_to_counts(signal, shots, channel_info):
    ...
    if len(signal) > 0:
        ...
        for ch in channel_id_pc:
            ...
            signal_out.loc[ch_d] = signal_out.loc[ch_d] * shots.loc[:,ch] * bin_temporal_resolution 
