...
def _as_time_channel_bins(arr, n_channels, n_times):
    """Convert a 3D Tamarin profile array to (time, channel, bins)."""

    arr = np.asarray(arr)
    if arr.ndim != 3:
        raise FileReaderError(f"--Expected a 3D Tamarin signal array, got shape {arr.shape}")

    axes = list(range(arr.ndim))

    channel_matches = [ax for ax in axes if arr.shape[ax] == n_channels]
    if len(channel_matches) == 0:
        raise FileReaderError(f"--Could not identify channel axis in Tamarin signal shape {arr.shape}")
    channel_axis = channel_matches[0]

    time_matches = [ax for ax in axes if arr.shape[ax] == n_times and ax != channel_axis]
    if len(time_matches) == 0:
        # Common single-time ambiguity: choose the first non-channel axis.
        time_matches = [ax for ax in axes if ax != channel_axis]
    time_axis = time_matches[0]

    bin_axis = [ax for ax in axes if ax not in [time_axis, channel_axis]][0]

    return np.transpose(arr, (time_axis, channel_axis, bin_axis))
...
def _laser_shots_to_time_channel(shots_var, n_channels, n_times):
    ...
    for ax, dim in enumerate(dims):
        if dim in ["time", "time_bck"] or "time" in dim:
            time_axis = ax
        if dim in ["points", "point", "channel", "channels", "channel_id"] or "point" in dim or "channel" in dim:
            channel_axis = ax
...
def read_signals_tamarin(nc_file, time, channels, meas_type):
    ...
    for mode, var_name in variables:
        if var_name is not None and var_name in group.variables:
            sig_arr = np.asarray(group[var_name].values).astype(float)
            sig_arr = _as_time_channel_bins(sig_arr, n_channels = n_channels, n_times = n_times)
            template = sig_arr
        else:
            if template is None:
                ...
                for _, candidate in variables:
                    if candidate is not None and candidate in group.variables:
                        template = _as_time_channel_bins(
                            np.asarray(group[candidate].values).astype(float),
