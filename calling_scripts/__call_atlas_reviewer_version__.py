...
# -----------------------------------------------------------------------------
config_ids = dict(example_name_1 = '0_0_0',
                  example_name_2 = '999_999_999')

# Station ID (e.g. 'aky', 'brc') 
st_id = ''

# Configuration name (same in the config_ids dictionary)
cfg = 'brc_night_new'
...
# Use True/False to Enable/Disable quick run (generate only plots)
quick_run = False
...
# Embed all channelplots in the html (e.g. also analog Rayleigh fits)
export_all = False
...
# -----------------------------------------------------------------------------
# Config file folder
config_folder = os.path.join(main_data_folder, st_id, 'configurations')

# Config file folder
settings_folder = os.path.join(main_data_folder, st_id, 'settings')

# Radiosonde folder
rsonde_folder = os.path.join(main_data_folder, st_id, 'radiosondes', config_ids[cfg].split('_')[0])

# Parent folder
if len(date) > 0:
    parent_folder = os.path.join(main_data_folder, st_id, f'{config_ids[cfg]}_{date}', subfolder)
else:
    parent_folders = np.sort(glob.glob(os.path.join(main_data_folder, st_id, f'{config_ids[cfg]}*')))
    if len(parent_folders) == 0:
        raise Exception(f'-- Error: No folders found inside the main_data_folder {os.path.join(main_data_folder, st_id)} matching the provided configuration ID {config_ids[cfg]}')
    elif len(parent_folders) > 1: 
        raise Exception(f'-- Error: More than one folders found inside the main_data_folder {os.path.join(main_data_folder, st_id)} matching the provided configuration ID {config_ids[cfg]}. {parent_folders}\n Please provide the date variable in order to select one of them')
    else:
...
# Configuration file
config_file = os.path.join(config_folder, f'config_file_{config_ids[cfg]}.ini')

# Settings file
settings_file = os.path.join(settings_folder, f'settings_file_{config_ids[cfg]}.ini')
...
with open(html_file, 'w') as f:
    ...
    f.write(f'<h1>QA report {st_id} {config_ids[cfg]}_{date} {expert_analyst} {now}</h1>')
    f.write('\n')
