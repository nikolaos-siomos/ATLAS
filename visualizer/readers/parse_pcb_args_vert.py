...
def call_parser():
    ...
    parser.add_argument('--y_lims_calibration', metavar = 'y_lims_calibration',
                        ...
                        help = 'The y axis limits in km (lower and upper) for the pcb. calibration plot. If use_distance is called, the limits correspond to distance. Defaults to 0 km (lower) and 14 km (upper) If values below 0 or above the maximum signal altitude/distance are used, they will be ignored')

    parser.add_argument('--y_tick_calibration', metavar = 'x_tick_calibration',
                        type = int, nargs = '?', default = 0.5, 
                        help = 'The y axis finest tick in km for the pcb. calibration plot. Defaults to 0.5km ')
...
def check_parser(args):
    ...
    if args['output_folder'] == None:
        out_path = os.path.join(os.path.dirname(args['input_file']),'..','..', 'plt')
