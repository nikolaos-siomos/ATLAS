...
from utils.error_classes import SignalError
from typing import Any, Dict, List, Tuple
from helper_functions.printouts import print_header, print_subsection


def detect_saturation(caller_info: Dict[str, Any], profiles: Dict[str, Any], 
                      metadata: Dict[str, Any], profile_masks: Dict[str, Any]) -> None:
    ...
    print_header('Detect saturation and clipping')
    ...
    for key in profiles.keys():
        ...
        channels = channel_info.channel.values
        ...
        mask_photon_saturated = (sig_clean / shots > max_counts) & (aq_mode == "p")
        ...
        for ch in channels:
            ...
            if mask_saturated_ch.loc[ch] and aq_mode.loc[ch] == "a":
                print("")
                print(f"-- Warning: Channel {ch} - Analog signal mV values above 80% of the data acqusition range were detected! ")   
    
            if mask_saturated_ch.loc[ch] and aq_mode.loc[ch] == "p":
                print("")
                print(f"-- Warning: Channel {ch} - Photon signal values above 1 count per bin per shot were detected! ")

        print("")

        if debug_signals:
            profile_masks[key]["saturated"] = mask_saturated
