...
def partition_function_ideal_rigid_rotor(molecular_parameters, temperature = 293.15):
    ...
    # Analytical partition function, valid for idea rigid rotor (D = 0)
...
def delta_mol_by_summing(ds_isotropic, ds_anisotropic):
    
    delta_m = 3. * ds_anisotropic / (7. * ds_isotropic + 4. * ds_anisotropic)
...
class RotationalRaman:
    ...
    def __init__(self, wavelength, temperature, max_J=40, N2_parameters=None, O2_parameters=None, Ar_parameters=None, CO2_parameters=None, H2O_parameters=None, optical_filter = None, istotal = False):
       ...
       temperature: float
        ...
        self.dn_stokes_O2 = [raman_shift_stokes(J, self.O2_parameters) for J in self.Js]
        self.dn_astokes_O2 = [raman_shift_antistokes(J, self.O2_parameters) for J in self.Js]
        ...
        # Calculate the Raman shift for the Ο2 lines
        self.dn_stokes_CO2 = [raman_shift_stokes(J, self.CO2_parameters) for J in self.Js]
        self.dn_astokes_CO2 = [raman_shift_antistokes(J, self.CO2_parameters) for J in self.Js]
    ...
    def quantum_mechanic_cabannes_cross_section(self, pressure = 1013.25):
        ...
        if optical_filter:
            ...
            ds_N2 = np.nansum(self.ds_Q_N2) + np.nansum(self.ds_isotr_N2)
            
            ds_O2 = np.nansum(self.ds_Q_O2) + np.nansum(self.ds_isotr_O2)

            ds_CO2 = np.nansum(self.ds_Q_CO2) + np.nansum(self.ds_isotr_CO2)
    ...
    def quantum_mechanic_isotropic_cross_section(self, pressure = 1013.25):
        ...
        if optical_filter:
            ds_N2 = optical_filter(self.wavelength) * np.nansum(self.ds_isotr_N2) 
        
            ds_O2 = optical_filter(self.wavelength) * np.nansum(self.ds_isotr_O2) 
                
            ds_CO2 = optical_filter(self.wavelength) * np.nansum(self.ds_isotr_CO2) 
            ...
            ds_N2 = np.nansum(self.ds_isotr_N2)
            
            ds_O2 = np.nansum(self.ds_isotr_O2)

            ds_CO2 = np.nansum(self.ds_isotr_CO2)

            ds_Ar = np.nansum(self.ds_isotr_Ar) 
        
            ds_H2O = np.nansum(self.ds_isotr_H2O) 
        
        cross_section_iso_gas = np.array([ds_N2, ds_O2, ds_Ar, ds_CO2, ds_H2O])
        ...
        cross_section_iso = np.nansum(c * cross_section_iso_gas)

        sigma_iso = N * cross_section_iso
        
        cross_sections_iso = dict(zip(gases, cross_section_iso_gas))
    ...
    def quantum_mechanic_anisotropic_cross_section(self, pressure = 1013.25):
        ...
        cross_section_aniso_gas = np.array([ds_N2, ds_O2, ds_Ar, ds_CO2, ds_H2O])
        ...
        cross_section_aniso = np.nansum(c * cross_section_aniso_gas)

        sigma_aniso = N * cross_section_aniso
        
        cross_sections_aniso = dict(zip(gases, cross_section_aniso_gas))

        return cross_section_aniso, cross_sections_aniso, sigma_aniso 
    ...
    def delta_mol_rayleigh(self,method):
        ...
        if method == 'fractional_contribution':
            ...
            cross_section_iso, _, _ = self.quantum_mechanic_isotropic_cross_section()
            cross_section_aniso, _, _ = self.quantum_mechanic_anisotropic_cross_section()
            delta_m = delta_mol_by_summing(ds_isotropic = cross_section_iso, ds_anisotropic = cross_section_aniso)
    ...
    def delta_mol_cabannes(self,method):
        ...
        if method == 'fractional_contribution':
            ...
            cross_section_iso, _, _ = self.quantum_mechanic_isotropic_cross_section()
            cross_section_aniso, _, _ = self.quantum_mechanic_anisotropic_cross_section()
            cross_section_rr, _, _ = self.quantum_mechanic_rr_cross_section()

            delta_m = delta_mol_by_summing(ds_isotropic = cross_section_iso, ds_anisotropic = cross_section_aniso - cross_section_rr)
